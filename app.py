...
def init_db():
    ...
    if c.execute("SELECT COUNT(*) FROM donors").fetchone()[0] == 0:
        sample_donors = [
            ...
            ("Priya Menon",     "A+",  "9447123402", "Kakkanad, Kochi",    32, "2024-11-20", 8,  10.0159, 76.3419),
            ("Rahul Nair",      "B+",  "9447123403", "Edappally, Kochi",   25, "2024-09-10", 3,  10.0205, 76.3100),
            ("Sneha Thomas",    "O-",  "9447123404", "Vytilla, Kochi",     29, "2024-12-01", 12, 9.9561,  76.3150),
            ("Vijay Krishnan",  "AB+", "9447123405", "Aluva, Kochi",       35, "2024-08-22", 6,  10.0998, 76.3573),
            ("Ananya Jose",     "A-",  "9447123406", "Fort Kochi, Kochi",  27, "2025-01-10", 2,  9.9637,  76.2430),
            ("Kiran Pillai",    "B-",  "9447123407", "Kalamassery, Kochi", 31, "2024-07-30", 9,  10.0524, 76.3141),
            ("Divya Mathew",    "AB-", "9447123408", "Thrikkakara, Kochi", 24, "2025-02-01", 1,  10.0092, 76.3395),
        ]
...
def submit_request():
    ...
    system = """You are LifeLink AI, an emergency blood allocation system for hospitals in Kochi, Kerala.
...
Current stock of {b['blood_type']}: {stock} units"""
    ...
    conn.commit()
    conn.close()
    return jsonify({"ok": True, "urgency": urgency, "ai_recommendation": ai_rec, "request_id": req_id})
...
def predict():
    ...
    system = """You are LifeLink AI shortage predictor for a blood bank in Kochi, Kerala.
